...
export const useComments = (userId: string, limit: number = 10) => {
  ...
  const {
    ...
    queryKey: ['profile-comments', userId, 1],
    queryFn: () => getComments(userId, limit, 1),
  });
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const data = await getComments(userId, limit, nextPage);
