...
function PinnedCard({ post, onClick, className }: PinnedCardProps) {
  ...
  const { selectedTags, toggleTag } = useProfileFilterStore();
  ...
  return (
    <Card
      ...
      className={cn(
        ...
        // flex-none makes sure the card doesn't shrink in height and maintains its aspect ratio
        // `snap-start` aligns the card to the start of the scroll container when snapping
        ...
        <CardContent className="mt-1 flex items-center gap-2 p-0 text-sm">
          {post.tags.slice(0, 2).map((tag: Tag) => (
            <Badge
              key={tag.tagId}
              className={cn(
                'inline-flex cursor-pointer items-center rounded-full p-1.5 text-xs transition-colors',
                selectedTags.has(tag.tagId)
                  ? 'bg-primary text-primary-foreground hover:bg-primary/80'
                  : 'bg-primary/60 text-muted hover:bg-primary',
              )}
              onClick={(e) => {
                e.stopPropagation();
                toggleTag(tag.tagId);
